...
def eval_alignment_by_sim_mat(embed1, embed2, top_k, nums_threads, f, csls=0, accurate=False,output = True):
    t = time.time()
    ...
    if output:
        if accurate:
            ...
            f.writelines("accurate results: hits@{} = {}, mr = {:.3f}, mrr = {:.3f}, time = {:.3f} s ".format(top_k, acc, t_mean,  t_mrr,  time.time() - t))
            f.writelines("\n")
        else:
...
def pairwise_distances(x, y=None):
    '''
    Input: x is Match ，1000 ，原始参数 Nxd matrix
           y is an optional Mxd matirx
    Output: dist is Match ，1000 ，原始参数 NxM matrix where dist[i,j] is the square norm between x[i,:] and y[j,:]
